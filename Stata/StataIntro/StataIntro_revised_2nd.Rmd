...
3. Set working directory, read and save data 
</div>
...
![](C:\Users\yiw640\Desktop\dss-workshops\Stata\StataIntro\images\StataInterface.png)
...
<div class="alert alert-info">
**GOAL: To learn about the working directory in Stata, open and save data.** In particular:

1. Print the current working directory and change it if necessary 
2. Open and save data in Stata 
3. Import and export data in Stata
</div>
...
We are using The Generations of Talent Study (``talent.dta``) to practice reading in data, and plotting data, and performing descriptive statistics. 
...
<div class="alert alert-info">
**GOAL: Create a new variable based on values of existing variables/recode a variable** 
Using `generate` and `replace` allow you to do that
</div>
...
+The **tabulate** (may be abbreviated as **tab**) command produces one- or two-way frequency tables given one or two variable
+The **tabi** is used to re-analyze a published table without access to the raw data
+The commands also can run a Chi-square test using the *chi2* option:
...
Now we find that the means are different, but we are interested only in testing whether the means for the North (region==1) and South (region==4) are different. We could use oneway:
